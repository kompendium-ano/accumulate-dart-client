...
class ACMEClientV3 {
  ...
  Future<Map<String, dynamic>> queryData(dynamic url, [String? entryHash]) async {
  ...
  Future<Map<String, dynamic>> queryUrl(dynamic url, [QueryOptions? options]) async {
  ...
  Future<Map<String, dynamic>> queryMinorBlocks(String url, QueryPaginationForBlocks pagination, MinorBlocksQueryOptions? options) {
    Map<String, dynamic> params = {};
  ...
  Future<Map<String, dynamic>> queryMajorBlocks(String url, QueryPaginationForBlocks pagination, MinorBlocksQueryOptions? options) {
    Map<String, dynamic> params = {};
    params.addAll({"url": url});
    params.addAll(pagination.toMap);
    if (options != null) {
  ...
  Future<Map<String, dynamic>> createDataAccount(dynamic principal, CreateDataAccountParam createDataAccount, TxSigner signer) {
  ...
  Future<Map<String, dynamic>> createTokenAccount(dynamic principal, CreateTokenAccountParam createTokenAccount, TxSigner signer) {
  ...
  Future<Map<String, dynamic>> updateAccountAuth(dynamic principal, UpdateAccountAuthParam updateAccountAuthParam, TxSigner signer) {
  ...
  Future<Map<String, dynamic>> updateKeyPage(dynamic principal, UpdateKeyPageParam updateKeyPageParam, TxSigner signer) {
    return _execute(AccURL.toAccURL(principal), UpdateKeyPage(updateKeyPageParam), signer);
  }

  Future<Map<String, dynamic>> updateValidatorKey(dynamic principal, UpdateValidatorKeyParam updateValidatorKey, TxSigner signer) {
    ...
    if (res != null) {
      ...
      switch (type) {
        ...
        case "sendTokens":
          ...
          tx = txModel.Transaction("Outgoing", "send_token", txid, from, urlRecipient, int.parse(amount!), "acc://");
          tx.created = DateTime.fromMicrosecondsSinceEpoch(ts!).millisecondsSinceEpoch;
          break;
    ...
    for (var i = 0; i < records.length; i++) {
      ...
      switch (type) {
        case "faucet": // that's faucet
          String? txid = tx["txid"];
          String? amount = tx["data"]["amount"]; // AMOUNT INCONSISTENT, faucet returns String while other types int
        ...
        case "sendTokens":
          ...
          txModel.Transaction txl = txModel.Transaction("Outgoing", "send_token", txid, from, urlRecipient, int.parse(amount!), "acc://");
          txl.created = DateTime.fromMicrosecondsSinceEpoch(ts!).millisecondsSinceEpoch;
          txs.add(txl);
          break;
        case "syntheticCreateChain":
  ...
  Future<Map<String, dynamic>> createLiteDataAccount(dynamic principal, CreateLiteDataAccountParam createLiteDataAccountParam, TxSigner signer) {
